...
public class MovieController {
    ...
    public void getDataSearch(String query, int page) {
        Call<MovieResponse> movieResponseCall = App.getInstance().getApiService().getSearchMovies(query, page, RestApi.getMoviesSearchOptional());
    ...
    public void getMovies(String movieType, int page) {
        Call<MovieResponse> movieResponseCall = App.getInstance().getApiService().getMovies(movieType, page, RestApi.getMoviesOptional());
        movieResponseCall.enqueue(new Callback<MovieResponse>() {
            @Override
            public void onResponse(Call<MovieResponse> call, Response<MovieResponse> response) {
                if (response.code() == 200)
    ...
    public void getMovieDetail(int movieId) {
        Call<Movie> movieCall = App.getInstance().getApiService().getMovieDetails(movieId, RestApi.getMovieDetailOptional());
